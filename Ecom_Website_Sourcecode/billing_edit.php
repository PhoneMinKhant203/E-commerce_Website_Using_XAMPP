...
// Fetch the current billing address, shipping address, and phone number
try {
    $stmt = $pdo->prepare("SELECT name, billing_address, shipping_address, phone FROM users WHERE id = :id");
...
// Update billing and shipping addresses if form is submitted
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $newBillingAddress = htmlspecialchars(trim($_POST['billing_address']));
    $newShippingAddress = htmlspecialchars(trim($_POST['shipping_address']));
    $newPhone = htmlspecialchars(trim($_POST['phone']));

    try {
        $stmt = $pdo->prepare("UPDATE users SET billing_address = :billing_address, shipping_address = :shipping_address, phone = :phone WHERE id = :id");
        $stmt->bindParam(':billing_address', $newBillingAddress, PDO::PARAM_STR);
        $stmt->bindParam(':shipping_address', $newShippingAddress, PDO::PARAM_STR);
        ...
        // Refresh values to reflect new changes
        $billingAddress = $newBillingAddress;
        $shippingAddress = $newShippingAddress;
        $phone = $newPhone;
    } catch (Exception $e) {
        echo "Error updating addresses: " . $e->getMessage();
    }
}
?>
...
<body>
    ...
    <div class="account_hero">
        ...
        <div class="Address_session_edit">
            <a href="address_details.php">Back</a>
            <div class="billing_edit">
                <h1>Edit Billing Address</h1>
                <form action="billing_edit.php" method="post">
                    <p>Name</p>
                    <input type="text" id="user_name" name="user_name" value="<?php echo htmlspecialchars($userName); ?>" readonly>

                    <p>Billing Address</p>
                    <input type="text" id="billing_address" name="billing_address" value="<?php echo htmlspecialchars($billingAddress); ?>" required>
        ...
        <div class="Address_session_edit">
            ...
            <div class="billing_edit">
                ...
                <input type="text" id="shipping_address" name="shipping_address" value="<?php echo htmlspecialchars($shippingAddress); ?>" required>

                <p>Phone</p>
                <input type="text" id="phone" name="phone" value="<?php echo htmlspecialchars($phone); ?>" required>

                <br><br><button type="submit" class="save_button">Save Changes</button>
                </form>
            </div>
        </div>
